...
public class Articulo {
	...
	public ArticuloDTO toDTO() {
		...
		for(Lote loteAux : this.getLotes()) {
			...
			loteDTO.setCodLote(loteAux.getCodLote());
			loteDTO.setFechaVtoLote(loteAux.getFechaVtoLote());
			
			for (Ubicacion ubiAux : loteAux.getUbicaciones()) {
				...
				ubicacionDTO.setNroUbicacion(ubiAux.getNroUbicacion());
				loteDTO.getUbicaciones().add(ubicacionDTO);
			}
		...
		for(Movimiento movAux : this.movimientos) {
			...
			movimientoDTO.setCant(movAux.getCant());
